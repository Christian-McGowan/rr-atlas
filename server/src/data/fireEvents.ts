...
const EVENTS: AtlasEvent[] = [
  ...
  },
  {
    id: "fl_fullerton_1",
    hazard: "flood",
    title: "Fullerton Creek Flood Watch (Demo)",
    lat: 33.871,
    lng: -117.925,
    status: "monitoring",
    acres: 0,
    updatedAt: iso(now - 90 * 60 * 1000),
    source: "Demo feed"
  },
  {
    id: "wf_fullerton_1",
    hazard: "wildfire",
    title: "North Fullerton Brush Fire Watch (Demo)",
    lat: 33.918,
    lng: -117.866,
    status: "contained",
    acres: 84,
    updatedAt: iso(now - 12 * 24 * 60 * 60 * 1000),
    source: "Demo feed"
  },
  {
    id: "wf_irvine_1",
    hazard: "wildfire",
    title: "Bommer Canyon Fire Watch (Demo)",
    lat: 33.632,
    lng: -117.786,
    status: "monitoring",
    acres: 145,
    updatedAt: iso(now - 3 * 60 * 60 * 1000),
    source: "Demo feed"
  },
  {
    id: "fl_irvine_1",
    hazard: "flood",
    title: "San Diego Creek Flood Advisory (Demo)",
    lat: 33.662,
    lng: -117.82,
    status: "contained",
    acres: 0,
...
export function getEventsForPlace(
  placeSlug: string,
  opts: { hazard: "any" | "wildfire" | "flood" | "storm" }
) {
  const place = getPlaceBySlug(placeSlug);
  if (!place?.bounds) return [];
